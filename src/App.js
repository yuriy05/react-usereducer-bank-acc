...
function reducer(state, action) {
  switch(action.type) {
    ...
    case "withdraw":
      return {
        ...state,
        balance: state.balance < 50 ? state.balance : state.balance - 50,
      }
    case "requestLoan":
      return {
        ...state,
...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <p>
        <button onClick={() => {dispatch({type: "requestLoan"})}} disabled={!isActive || loan}>
          Request a loan of 5000
        </button>
      </p>
      <p>
        <button onClick={() => {dispatch({type: "payLoan"})}} disabled={!isActive || !loan}>
          Pay loan
        </button>
      </p>
      <p>
